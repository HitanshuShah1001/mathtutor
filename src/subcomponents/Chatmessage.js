...
export const ChatMessage = ({ message, role, mediaUrl }) => {
  ...
  const handleCopy = () => {
    const textToCopy = Array.isArray(message)
      ? message.filter((item) => item.type === "text").map((item) => item.text).join("\n")
      : message;
        ...
        components={{
          code({ node, inline, className, children, ...props }) {
            ...
            return !inline && match ? (
              ...
              <code className={`${className} bg-indigo-50 text-indigo-600 rounded-md px-2 py-0.5 text-sm font-mono`} {...props}>
                {children}
              </code>
            );
          },
  ...
  return (
    <div className={`py-8 w-full ${role === "user" ? "bg-gradient-to-r from-indigo-50/50 to-purple-50/50" : "bg-white"}`}>
      <div className="flex gap-6 relative group px-6">
        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-sm transform transition-transform duration-200 hover:scale-110 
          ${role === "USER" ? "bg-gradient-to-br from-indigo-500 to-purple-600" : "bg-gradient-to-br from-emerald-500 to-teal-600"}`}>
          {role !== ASSISTANT ? (
        ...
        <div className="flex-1 min-w-0">
          {mediaUrl && (
            <div className="my-4">
              <img src={mediaUrl} alt="Uploaded Content" className="rounded-2xl shadow-lg max-w-full" />
            </div>
          )}
        ...
        >
          <CopyIcon className="text-gray-400 hover:text-indigo-600 transition-colors" size={18} />
          {copied && (
